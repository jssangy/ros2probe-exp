...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  cat <<'USAGE'
Usage: run_bag_subscribers.sh --sync <peer-IP> [options]
...
# Receiver platform - Experiment 2 (resource overhead) bag-record runner
#
# Usage:
#   ./experiments/02_resource_overhead/scripts/run_bag_subscribers.sh --sync <Publisher-IP> --platform <pc|rpi|jetson>
#   ./experiments/02_resource_overhead/scripts/run_bag_subscribers.sh --sync <Publisher-IP> --platform rpi --scenarios ST100 --conditions rp_bag,rosbag2 --runs 10
...
run_one() {
  ...
  outdir="${RP_EXP_RESULTS_ROOT:-${REPO_DIR}/results}/02_resource_overhead/recording/${PLATFORM}/${scenario}/${condition}/run${run}"
  bagdir="${RP_EXP_BAGS_ROOT:-${REPO_DIR}/bags}/02_resource_overhead/${PLATFORM}/${scenario}/${condition}/run${run}"
  bag_mcap="${bagdir}/stress_bag_0.mcap"
  mkdir -p "${outdir}"
  rp_exp_begin_run "${outdir}"
  mkdir -p "${bagdir}"
  echo "[overhead_bag] ${PLATFORM}/${scenario}/${condition}/run${run}  NIC=${NIC}"
  ...
  case ${condition} in
    rosbag2)
      rm -f "${bag_mcap}"
      rosbag_tmp="${bagdir}/rosbag2"
      setsid env --default-signal=INT,QUIT ros2 bag record ${BAG_TOPICS} --storage mcap -o "${rosbag_tmp}" > "${outdir}/obs.log" 2>&1 &
      obs_pid=$!
      ;;
    rp_bag)
      rm -f "${bag_mcap}"
      ...
      setsid env --default-signal=INT,QUIT "${RP_BIN}" bag record ${BAG_TOPICS} -o "${bag_mcap}" >> "${outdir}/obs.log" 2>&1 &
      obs_pid=$!
      ;;
  esac
  ...
  run_succeeded=1
  cleanup_run
  if [[ -n "${rosbag_tmp}" ]]; then
    bag_mcap="${rosbag_tmp}"
  fi

  rp_exp_validate_bag "${bag_mcap}" "${TOPIC}"
  rp_exp_complete_run "${outdir}"
  trap - EXIT INT TERM
  ros2 daemon stop 2>/dev/null || true
  normalize_tty
  echo "[overhead_bag] run${run} done -> ${outdir}"
  sleep 10
}
...
echo "================================================"
echo " Experiment 2 (resource overhead) Bag - Receiver Platform"
echo " Platform  : ${PLATFORM}"
...
echo " Results   : ${RP_EXP_RESULTS_ROOT:-${REPO_DIR}/results}/02_resource_overhead/recording/${PLATFORM}/"
echo " Bag file  : ${RP_EXP_BAGS_ROOT:-${REPO_DIR}/bags}/02_resource_overhead/${PLATFORM}/<scenario>/<condition>/runXX/stress_bag_0.mcap"
...
echo ""
echo "================================================"
echo " Experiment 2 (resource overhead) Bag complete  $(date '+%H:%M:%S')"
