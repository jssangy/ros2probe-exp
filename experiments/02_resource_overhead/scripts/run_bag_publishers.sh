...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  cat <<'USAGE'
Usage: run_bag_publishers.sh --sync <peer-IP> [options]
...
# Publisher host - Experiment 2 (resource overhead) bag-record controller
#
# Usage:
#   ./experiments/02_resource_overhead/scripts/run_bag_publishers.sh --sync <Receiver-IP> [--platform <pc|rpi|jetson>]
#
...
echo "================================================"
echo " Experiment 2 (resource overhead) Bag - Publisher Host"
...
echo "Waiting for commands from Receiver. Start run_bag_subscribers.sh --sync <Publisher-IP> on receiver."
echo ""

while true; do
  TMP_CMD=$(mktemp /tmp/rp_overhead_bag_cmd.XXXXXX)
  ...
  case "${CMD}" in
    START\ *)
      ...
      PUB_LOG="${RP_EXP_RESULTS_ROOT:-${REPO_DIR}/results}/02_resource_overhead/publisher/bag_${SCENARIO}.log"
    ...
    DONE)
      echo ""
      echo "================================================"
      echo " Experiment 2 (resource overhead) Bag publisher complete  $(date '+%H:%M:%S')"
      echo "================================================"
      break
      ;;
    "")
