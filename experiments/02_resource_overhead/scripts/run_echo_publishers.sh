...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  cat <<'USAGE'
Usage: run_echo_publishers.sh --sync <peer-IP> [options]
...
# Publisher host - Experiment 2 (resource overhead) echo-observer controller
#
# Usage:
#   ./experiments/02_resource_overhead/scripts/run_echo_publishers.sh --sync <Receiver-IP> [--platform <pc|rpi|jetson>]
#
# Receiver sends "START <scenario>" / "STOP" / "DONE".
...
echo "================================================"
echo " Experiment 2 (resource overhead) Echo - Publisher Host"
...
echo "Waiting for commands from Receiver. Start run_echo_subscribers.sh --sync <Publisher-IP> on receiver."
echo ""

while true; do
  TMP_CMD=$(mktemp /tmp/rp_overhead_echo_cmd.XXXXXX)
  ...
  case "${CMD}" in
    START\ *)
      ...
      PUB_LOG="${RP_EXP_RESULTS_ROOT:-${REPO_DIR}/results}/02_resource_overhead/publisher/echo_${SCENARIO}.log"
    ...
    DONE)
      echo ""
      echo "================================================"
      echo " Experiment 2 (resource overhead) Echo publisher complete  $(date '+%H:%M:%S')"
      echo "================================================"
      break
      ;;
    "")
