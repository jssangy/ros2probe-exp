...
int main(int argc, char ** argv) {
  ...
  const std::string topic = args.size() > 1 ? args[1] : "/points";
#ifdef RELIABLE_QOS
  constexpr bool reliable = true;
#else
  constexpr bool reliable = false;
#endif
  auto node = std::make_shared<rp_exp::TimedSubscriber<sensor_msgs::msg::PointCloud2>>(
    "s3_points_subscriber", topic, 20,
